<template>
  <AssessmentLayout>
    <!-- HEADER -->
    <AssessmentHeader
      :title="`แบบประเมินเกี่ยวกับการรู้คิดและการควบคุมการรู้คิด`"
    ...
    <div v-if="step === 'practice'" class="w-full flex flex-col">
      ...
      >
        <div class="text-white text-[36px] font-medium">คำชี้แจง</div>
      </div>
      ...
      >
        <div class="text-center text-[24px]">
          เราต้องการทราบวิธีการเรียนและการทำงานที่นักเรียนใช้เมื่อส่งงานให้ครู
          ก่อนตอบคำถามแต่ละข้อ
          <br />
          ให้นักเรียนนึกถึงวิธีการเรียนที่ตนเองใช้
          และวิธีการทำงานที่ปฏิบัติเมื่อได้รับมอบหมายจากครู
          <br />
          จากนั้นให้นักเรียนอ่านแต่ละข้อ และเลือกว่าตนเอง “เห็นด้วย”
          ในระดับใดกับข้อความนั้น โดยมีระดับความคิดเห็นให้เลือก ได้แก่:
        </div>
        ...
        <div class="text-center text-[24px]">
          ...
          (Knowledge of Cognition) และการควบคุมการรู้คิด (Regulation of
          Cognition)
        </div>
      </div>
      ...
      <!-- TAB: ส่วนที่ 1 / 2 -->
      <div
        class="w-full flex items-center justify-between gap-[10px] text-black mt-[120px] mb-1"
      >
        <button
          v-for="sec in questionnaireSections"
          :key="sec.id"
      ...
      >
        <div
          ...
          :key="cat.id"
          class="w-full flex flex-col gap-8 mt-6"
        >
          ...
          >
            ...
            >
              <div
                v-for="opt in SCALE_OPTIONS"
                :key="opt.value"
                :class="scaleCardClasses(item.id, opt.value)"
                @click="setAnswer(item.id, opt.value)"
              >
    ...
    <AssessmentSummary
      ...
      :is-main-time-up="isMainTimeUp"
      :is-final-time-up="isFinalTimeUp"
...
function buildAnswersWithMissingFilled0() {
  const filled = { ...(answers.value || {}) };

  for (const item of allItems.value) {
    if (typeof filled[item.id] !== "number") {
      filled[item.id] = 0; // ✅ missing = 0
    }
  }
  return filled;
}
...
async function autoSubmitWithMissingAsZero() {
  ...
  try {
    // เติม missing เป็น 0 ก่อนส่ง
    const filled = buildAnswersWithMissingFilled0();
    answers.value = filled; // optional: ให้ UI สะท้อนด้วย

    await submitAll({ autoSubmitted: true }); // ปรับ submitAll ให้รับ option
    clearStorage();

    alert("หมดเวลา ระบบส่งแบบประเมินอัตโนมัติแล้ว");
    await navigateTo("/fuzzy-result?mode=pre");
  } catch (e) {
    console.error("❌ SRM auto submit failed:", e);
...
const buildSrmPayload = (answersOverride = null, extra = {}) => {
  const ans = answersOverride ?? answers.value;
  ...
  const timeUsedSeconds = Math.max(
    0,
    Math.floor((now - startedAtMs.value) / 1000)
  );
  ...
  return {
    ...
    averageScore,
    answers: ans,
    sectionScores: normalize(sectionAgg),
    categoryScores: normalize(categoryAgg),
    ...extra,
  };
};

const submitAll = async (opts = {}) => {
  const payload = buildSrmPayload(answers.value, opts);
