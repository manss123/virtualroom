<template>
    <div class="w-full flex flex-col items-center justify-start py-10 px-4 text-white">
        <div class="w-full max-w-6xl">
            <!-- Title -->
            <div class="text-center mb-6">
                <h1 class="text-3xl font-semibold mb-2">
                    วางแผนตารางเรียนกันครับ
                </h1>
                <p class="text-lg">
                    ให้นักเรียนเลือกวันที่ต้องการเริ่มเรียนและจบการเรียนในแต่ละห้องเรียน
                    โดยให้หมดเวลา การเรียนทุกห้องภายใน 1 เดือน (31 วัน) ก่อนเข้าร่วมแคมป์
                </p>
            </div>

            <!-- PLAN TABLE -->
            <div class="w-full bg-white rounded-nw shadow-md shadow-[#FFC233] px-8 pt-8 pb-16 text-black mb-10">
                <!-- table header -->
                <div class="grid grid-cols-3 gap-4 border-b border-gray-300 pb-4 mb-4 text-lg font-semibold">
            ...
            <!-- CALENDAR BLOCK -->
            <div class="w-full bg-white rounded-nw shadow-md shadow-[#FFC233] px-8 pt-8 pb-10 text-black">
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center gap-2 text-lg">
                        <button type="button" class="px-3 py-1 rounded bg-gray-200 hover:bg-gray-300"
                            @click="prevMonth">
                            ‹
                        </button>
                ...
                <div class="w-full border border-gray-300 rounded-lg overflow-hidden">
                    <!-- weekday header -->
                    <div class="grid grid-cols-7 bg-gray-100 text-center text-xs font-semibold py-2">
                        <div>SUN</div>
                        <div>MON</div>
                        <div>TUE</div>
                        <div>WED</div>
                    ...
                    <div class="grid grid-cols-7 text-center text-sm">
                        <<div v-for="(day, index) in calendarDays" :key="index"
                            ...
                            :class="[
                                ...
                                day.roomsKeys.length === 1 ? roomColorClass(day.roomsKeys[0]) : '',
                            ...
                            <div v-else-if="day.roomsKeys.length === 1" class="flex items-center gap-1">
                                <span class="w-2 h-2 rounded-full border border-white/60"
                                    :class="roomColorClass(day.roomsKeys[0])"></span>
                            </div>

                            <!-- multiple rooms: stacked dots -->
                            <div v-else class="flex items-center gap-1">
                                <span v-for="rk in day.roomsKeys.slice(0, 3)" :key="rk"
                                    class="w-2 h-2 rounded-full border border-white/60"
                                    :class="roomColorClass(rk)"></span>
                                <span v-if="day.roomsKeys.length > 3" class="text-[10px] text-gray-600">
            ...
            <div class="mt-4 flex flex-wrap gap-4 text-xs">
                <div v-for="room in rooms" :key="room.key + '-legend'" class="flex items-center gap-2">
                    <span class="w-4 h-4 rounded-sm border border-white/60" :class="roomColorClass(room.key)"></span>
                    <span>{{ room.label }}</span>
                </div>
            </div>

            <!-- overall duration info -->
            <div v-if="overallPlanDays > 0" class="mt-3 text-center text-sm text-black">
            ...
            <div v-if="hasOverlappingDay" class="mt-2 text-xs text-red-500 text-center">
        ...
        <div class="mt-8 flex items-center justify-center">
            <button type="button"
                ...
                :disabled="saving || isOverMaxDays" @click="savePlan">
...
const calendarDays = computed<CalendarCell[]>(() => {
    ...
    for (let i = 0; i < startWeekday; i++) {
        const date = new Date(currentYear.value, currentMonth.value, i - startWeekday + 1);
        days.push({
            date,
...
// ---------- 3. Overall duration constraint (≤ 31 days) ----------
