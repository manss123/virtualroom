<template>
    ...
    <div v-if="block.type === 'QT1'" class="flex items-center justify-center gap-14 pb-12 border-b border-black">
        <div class="w-fit h-fit bg-gray-300 rounded-nw flex items-center justify-center">
            <img v-if="block.image" :src="getImageURL(block.image)" alt="" class="max-w-690px h-auto object-contain" />
        </div>

        <div class="flex flex-col w-[763px]">
    ...
    <div v-else-if="block.type === 'QT2'"
        ...
        <div>{{ block.prompt }}</div>
        <div class="flex w-full items-center justify-evenly mt-8">
            <!-- <button >
                {{ opt.text }}
            </button> -->
    ...
    <div v-if="block.type === 'QT5'" class="flex items-center justify-center gap-14 pb-12 border-b border-black">
            ...
            <div class="flex w-full gap-4 mt-8">
                <button v-for="opt in block.options" :key="opt.value" type="button" @click="select(opt.value)"
                    ...
                    <img v-if="opt.image" :src="getImageURL(opt.image)" :alt="opt.value"
                        class="w-[340px] h-[285px] object-contain mb-2" />
    ...
    <div v-if="block.type === 'QT6'" class="flex items-center justify-center gap-14 pb-12 border-b border-black">
        <div v-if="block.image" class="w-[690px] h-[370px] bg-gray-300 rounded-nw flex items-center justify-center">
            <img v-if="block.image" :src="block.image" alt="" class="max-w-full max-h-full object-contain" />
        </div>
        ...
        <div class="flex flex-col w-full items-center justify-center">
            <div class="">{{ block.prompt }}</div>
            <div class="flex w-full flex-wrap items-center justify-center gap-x-10 gap-y-10 mt-8">
                <button v-for="opt in block.options" :key="opt.value" type="button" @click="select(opt.value)"
                    :class="[optionClasses(opt.value, 'flex-col w-4/12 min-h-[100px] h-fit'), opt.image ? ' p-2' : '']">
                    <img v-if="opt.image" :src="getImageURL(opt.image)" :alt="opt.value"
                        class="w-full h-[285px] object-contain mb-2" />
                    <!-- label เช่น ก. ข. หรือข้อความ มั่นใจ/ไม่มั่นใจ -->
                    <span v-if="opt.text">
                        {{ opt.text }}
                    </span>
